...
### Unknown barcode: scan the nutrition label

When a barcode is not in the database, **Scan nutrition label** photographs the nutrition table on the package (also available as "Fill from a label photo" in the custom product form of the meal editor). The backend has the AI vision provider transcribe the table, in Russian or English, and converts it to values per 100 g (a per-serving table is converted with the printed serving size, kJ to kcal). The app then shows the custom product form filled in, with notes on anything converted or doubtful; **nothing is saved until you check the values and tap Create**, and a value the photo did not show stays empty. The product is stored on the device together with its barcode, so the next scan of that code works offline. The label photo goes to the AI provider like a meal photo and is not stored by CalSnap (see the [privacy note](docs/security/privacy.md)). A model can misread digits: always check them against the package. Details: [ADR 011](docs/adr/011-nutrition-label-reading.md).
...
* Label reading needs the AI provider (there is no on-device OCR: ML Kit has no Cyrillic model), can misread digits, and its `ENERGY_MISMATCH` check is a hint, not proof.
