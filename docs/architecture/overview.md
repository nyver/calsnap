...
* `internal/vision/gemini` – Gemini REST provider (structured output through `responseSchema`, key in a header, bounded response body). `internal/vision/fake` – deterministic canned results.
* `internal/nutrition` – the embedded catalog, name normalization, and the matching pipeline: exact id, alias, then fuzzy (token containment and Levenshtein similarity), falling back to the AI estimate. `NUTRITION_ESTIMATED` marks fallbacks.
