...
**Android release.** The base URL is a `--dart-define`; the app refuses to start in release mode with a non-HTTPS URL. Cleartext traffic is disabled in the main manifest (`usesCleartextTraffic="false"`), there is no network security config, and cloud backup is off.
