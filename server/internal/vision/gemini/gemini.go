...
// PromptVersion identifies the prompt text below. Bump it when the prompt changes.
...
func buildRequest(img analysis.Image, rc analysis.RequestContext) map[string]any {
	return map[string]any{
		...
		"contents": []any{map[string]any{
			"role": "user",
			"parts": []any{
				map[string]any{"text": userPrompt(rc)},
...
const systemPrompt = `You analyze one photo of a meal for a calorie diary.
...
func userPrompt(rc analysis.RequestContext) string {
	lang := "English"
