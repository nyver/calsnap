...
rem
rem   debug    Android debug APK.
rem   release  (default) Android release APKs split per ABI.
rem
rem The backend address is set in the app (Settings > Server address). Environment variables (all optional):
rem   API_BASE_URL         default address used until one is set in the app; must be https:// for release
...
set "URL_DEFINE=--dart-define=API_BASE_URL=%API_BASE_URL%"
...
if /i "%MODE%"=="debug" (
    call flutter build apk --debug %URL_DEFINE% --dart-define=APP_VERSION=%VERSION%
    if errorlevel 1 goto fail
    ...
    call flutter build apk --release --split-per-abi %URL_DEFINE% --dart-define=PRIVACY_POLICY_URL=%PRIVACY_POLICY_URL% --dart-define=APP_VERSION=%VERSION%
    if errorlevel 1 goto fail
